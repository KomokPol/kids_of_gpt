      ...
      <div className={styles.left}>
        <div className={styles.leftHeader}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            >←</button>
            <Text variant="h3">Moshennik<Text variant="h3" as="span" color="accent">AI</Text></Text>
          </div>
          <Text variant="caption" color="muted">AI-наставник по понятиям</Text>
        </div>
      ...
      <div className={styles.right}>
        ...
        <div className={styles.messages}>
          {messages.map(msg => (
            <div
              key={msg.id}
              className={[styles.message, msg.role === 'user' ? styles.messageUser : styles.messageAi].join(' ')}
            >
              {msg.role === 'ai' && <span className={styles.messageAvatar}>🤖</span>}
        ...
        <div className={styles.inputRow}>
          <textarea
            ...
            disabled={typing}
          />
          <Button variant="primary" size="md" onClick={handleSend} disabled={!input.trim() || typing}>
            →
          </Button>
        </div>
